...
void NunoMesh::Initialize() {
    ...
    for (int y = 0; y <= kSubdivision_; ++y) {
        ...
        for (int x = 0; x <= kSubdivision_; ++x) {
            ...
            meshPoints_[y][x].position = Vector2{ px, py };
            meshPoints_[y][x].velocity = Vector2{ 0.0f, 0.0f };
    ...
    for (int y = 0; y <= kSubdivision_; ++y) {
        for (int x = 0; x <= kSubdivision_; ++x) {
            if (x + 1 <= kSubdivision_) {
                PBD::Constraint c;
                c.prevIndex = y * (kSubdivision_ + 1) + x;
                c.nextIndex = y * (kSubdivision_ + 1) + (x + 1);
                c.distance = Length(meshPoints_[y][x].position - meshPoints_[y][x + 1].position);
...
void NunoMesh::Update() {
    ImGui::SliderFloat3 ("Point[0][0]",&meshPoints_[0][0].position.x,0.01f, 1.0f, "%.3f");
    ImGui::SliderFloat3 ("Point[kSubdivision_][kSubdivision_]",&meshPoints_[kSubdivision_][kSubdivision_].position.x,0.01f, 1.0f, "%.3f");
}
...
void NunoMesh::Draw(
    ...
    for (int y = 0; y <= kSubdivision_; ++y) {
        for (int x = 0; x <= kSubdivision_; ++x) {
            // 3D→2D変換
            Vector3 pos3d = Vector3{ meshPoints_[y][x].position.x,meshPoints_[y][x].position.y,0.0f};
